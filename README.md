...
Текущая версия парсера понимает объявления module/import/let|const|var/func/struct/interface и модификатор `export` перед любым top-level объявлением, поэтому CLI подойдёт для быстрых проверок синтаксиса этих конструкций. Добавлен режим предварительного IR: `impulsec --file main.imp --emit-ir` печатает текстовое представление модуля (binding'и, функции, структуры и интерфейсы, включая признак `export`) прямо в терминал. Для простых семантических проверок (дубликаты имён, поля структур и сигнатур методов интерфейсов) используйте `impulsec --file main.imp --check`. Чтобы увидеть реальные числовые значения, которые фронтенд вычисляет на этапе компиляции, добавлен флаг `--evaluate`: `impulsec --file main.imp --evaluate` исполняет IR-интерпретатор для всех top-level binding'ов и печатает `name = value`, а `--eval-binding foo` выводит только выбранный binding и объясняет, почему он не смог вычислиться (например, «depends on unevaluated bindings»).

### Нативный C++ CLI

Чтобы не зависеть от Go-инструментов, собрали маленький C++-драйвер `impulse-cpp`, который использует те же библиотеки фронтенда и понимает те же флаги:

```bash
cmake --build build --target impulse-cpp
./build/tools/cpp-cli/impulse-cpp --file path/to/module.imp [--emit-ir] [--check] [--evaluate]
```

Поведение идентично Go-версии: без флагов печатается `Parse successful`, `--emit-ir` выводит текстовое IR, `--check` запускает семантический проход и сообщает об ошибках, а `--evaluate` печатает значения binding'ов (или причину, почему они не вычислились).

## MVP статуса тулчейна

Минимально рабочая версия включает полный поток `source → AST → semantic → IR → tooling`:

- **Парсер и AST**: модульные заголовки, импорты, `let|const|var`, функции, структуры и интерфейсы с модификатором `export`.
- **Выражения**: бинарные и литеральные выражения для инициализаторов разбираются в AST, печатаются обратно при lowering, вычисляются до констант и транслируются в стековые IR-инструкции (`literal`, `reference`, `binary`, `store`).
- **Диагностика**: аккумулируем `Diagnostic` с точным `line:column`, CLI выводит стек ошибок.
- **Семантика констант**: `const` binding'и обязаны иметь вычислимый на этапе компиляции инициализатор; ошибки (например, деление на ноль) ловятся ещё до IR.
- **IR-симулятор**: мини-интерпретатор выполняет стековые инструкции binding'ов (используется в тестах), поэтому текстовый IR подтверждён реальным исполнением значений.
- **Семантика**: проверяем дубликаты импортов, alias'ов, binding'ов, полей структур и методов интерфейсов.
- **IR**: `frontend::lower_to_ir` формирует текстовое представление, CLI умеет печатать через `--emit-ir`.
- **CLI**: Go `impulsec` и C++ `impulse-cpp` оборачивают те же библиотеки и работают одинаково.

### Как прогнать MVP end-to-end

```bash
# C++ модульные тесты (ctest теперь знает про impulse-tests)
cmake --build build
cd build && ctest
...
# Smoke: парсинг, проверка и IR из CLI
./impulsec --file path/to/module.imp --check
./impulsec --file path/to/module.imp --emit-ir
```

Используйте любой модуль, удовлетворяющий грамматике из `docs/spec/grammar.md`: CLI сообщает об ошибках синтаксиса и семантики и печатает IR без дополнительных шагов.

При `--emit-ir` числовые инициализаторы binding'ов теперь сопровождаются комментарием `# = <значение>`, показывающим результат вычисления выражения (например, `let value: int = (1 + (2 * 3));  # = 7`).
